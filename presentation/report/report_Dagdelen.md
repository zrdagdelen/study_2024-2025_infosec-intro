...
*Преимущества использования SELinux в Linux:* [[8]](https://www.explinux.com/2023/01/what-is-selinux.html)

- *Улучшенная безопасность.* SELinux обеспечивает дополнительный слой защиты путём принудительного контроля доступа. 
- *Тонко настраиваемый контроль.* Решения принимаются на основе всей доступной информации, такой как пользователи SELinux, роли, тип и уровень. 
- *Улучшенная целостность системы.* SELinux помогает предотвратить несанкционированные изменения в системных файлах и конфигурациях. 
- *Соответствие нормативным требованиям.* SELinux помогает соответствовать нормативным требованиям к безопасности. 
- *Уменьшение уязвимостей.* SELinux помогает снизить риск уязвимостей в системе, ограничивая доступ к чувствительным файлам и процессам. 
- *Улучшенная изоляция.* SELinux помогает изолировать разные процессы и не позволяет им вмешиваться друг в друга. 
- *Улучшенный аудит.* SELinux предоставляет подробные логи активности системы, что упрощает аудит и мониторинг системы. 

*Недостатки использования SELinux в Linux:*[[1]](https://habr.com/ru/companies/kingservers/articles/209644/)

- *Сложности с настройкой.*  Настройка достаточно сложна и невозможна без изучения специального языка конфигурации. 
-  *Трудозатраты системных администраторов.* Конфигурирование системы в жёстком режиме SELinux сопряжено с большими трудозатратами.
- *Риск ограничения доступа.* Пользователи могут столкнуться с ограничением доступа, если захотят использовать систему нетривиальным образом.

Выбор между преимуществами и недостатками использования SELinux зависит от конкретных условий и требований к системе.

## Сравнение с другими системами безопасности

1. SMACK. Предназначен для простых систем, которые не требуют высокого уровня безопасности. SMACK использует назначенные процессам и файлам метки безопасности для обеспечения политик контроля доступа. Некоторые особенности SMACK: [[9]](https://semiengineering.com/linux-security-primer-selinux-and-smack-frameworks/)

	- Ориентирован на простоту и гибкость. SMACK не включает себя по умолчанию в основных дистрибутивах, но может быть включён с помощью специальной сборки ядра.
	- Подходит для встроенных систем. SMACK — стандартная реализация контроля доступа в Automotive Grade Linux и Tizen. 

Таким образом, SELinux больше подходит для систем, требующих высокого уровня безопасности, а SMACK — для простых систем.

2. AppArmor. [[10]](https://wirekat.com/apparmor-vs-selinux-a-comparison/)

	- Механизм контроля доступа. AppArmor использует профили безопасности, основанные на путях, а SELinux — политики безопасности, основанные на метках файлов. 
	- Доступность и совместимость. AppArmor доступен для любого дистрибутива Linux, но в основном используется в системах SUSE и Ubuntu. SELinux также доступен для любого дистрибутива, но в основном используется в системах Red Hat и Fedora.
	- Сложность и гибкость. AppArmor проще в использовании, настройке и управлении, так как имеет более простой и интуитивно понятный синтаксис и команды. SELinux сложнее и менее удобен для пользователя, так как требует понимания концепций контекстов, типов, ролей и доменов.
	- Уровень защиты. И AppArmor, и SELinux обеспечивают высокий уровень защиты систем Linux, но уровень защиты зависит от качества и точности определённых для приложений профилей или политик. 
	- Подход к применению политик. AppArmor работает по принципу предоставления доступа сначала, а затем применения ограничений, в то время как SELinux работает по принципу отказа в доступе по умолчанию, а затем предоставления исключений
...
9. What is SELinux [[8]](https://www.explinux.com/2023/01/what-is-selinux.html)
10. Linux Security Primer: SELinux And SMACK Frameworks [[9]](https://semiengineering.com/linux-security-primer-selinux-and-smack-frameworks/)
11. Сравнение SELinux и AppArmor[[10]](https://wirekat.com/apparmor-vs-selinux-a-comparison/)
